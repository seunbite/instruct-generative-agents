...
def run_gpt_prompt_wake_up_hour(persona, test_input=None, verbose=False): 
  ...
  output = safe_generate_response(prompt, gpt_param, 5, fail_safe,
                                   __func_validate, __func_clean_up, prompt_template=prompt_template)
  
  if debug or verbose: 
    print_run_prompts(prompt_template, persona, gpt_param, 
                      prompt_input, prompt, output)
    
  return output, [output, prompt, gpt_param, prompt_input, fail_safe, prompt_template]
...
def run_gpt_prompt_generate_hourly_schedule(persona, 
  ...
  # seungwon edit : 
  # 다음 일정만 가져오고 나머진 버리도록! 
  print("~~~~~output of hourly planning~~~~")
  print(output)
  try:
    output = output.split("\n")[0]
    if output[-1]==".":
...
def run_gpt_prompt_task_decomp(persona, 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    print("run_gpt_prompt_task_decomp PROMPT")
    print (prompt)
    print("run_gpt_prompt_task_decomp RESPONSE")
    print (gpt_response)
    ...
    print("~prompt~~")
    print(prompt)
    print(prompt.split("(total duration in minutes")[-1]
                                   .split("):")[0].strip())
    total_expected_min = int(prompt.split("(total duration in minutes")[-1]
                                   .split("):")[0].strip())
    print("total_expected_min", total_expected_min)
    ...
    # print("curr_min_slot", curr_min_slot) 
    # print("len curr_min_slot", len(curr_min_slot))
...
def run_gpt_prompt_action_sector(action_description, 
  ...
  def __func_validate(gpt_response, prompt=""): 
    ...
    if "," in gpt_response: 
      return False
    if " or " in gpt_response: 
      return False
    if " and " in gpt_response: 
      return False
...
def run_gpt_prompt_pronunciatio(action_description, persona, verbose=False): 
  ...
  output = ChatGPT_safe_generate_response(prompt, example_output, special_instruction, 3, fail_safe,
                                          __chat_func_validate, __chat_func_clean_up, True, prompt_template=prompt_template)
  if output != False: 
    return output, [output, prompt, gpt_param, prompt_input, fail_safe, prompt_template]
  # ChatGPT Plugin ===========================================================
...
def run_gpt_generate_safety_score(persona, comment, test_input=None, verbose=False): 
  ...
  output = ChatGPT_safe_generate_response_OLD(prompt, 3, fail_safe,
                        __chat_func_validate, __chat_func_clean_up, verbose, prompt_template=prompt_template)
  print (output)
